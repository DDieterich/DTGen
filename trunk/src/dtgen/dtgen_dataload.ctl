...
--    July      28, 2012  09:54:36 AM
--
-- sqlldr username/password CONTROL=FILENAME
--
load data infile *
into table APPLICATIONS_ACT APPEND when key = 'APPLICATIONS                  ' fields terminated by '' TRAILING NULLCOLS
   (key FILLER position(1:31), abbr CHAR, name CHAR, db_schema CHAR, db_schema_exp CHAR, apex_schema CHAR, apex_ws_name CHAR, apex_app_name CHAR, dbid CHAR, db_auth CHAR, description CHAR, ts_null_override CHAR, ts_onln_data CHAR, ts_onln_indx CHAR, ts_hist_data CHAR, ts_hist_indx CHAR, usr_frgn_key CHAR, usr_datatype CHAR)
...
TAB_COLS                      DTGENAPPdb_schema3VARCHAR230UName of the database schema objects owner. Used for user synonym and DB Link creation. Also used for explicit owner of all database objects if db_schema_exp flag is set.
TAB_COLS                      DTGENAPPapex_schema5VARCHAR230UName of the APEX parsing schema owner for the generated APEX pages
TAB_COLS                      DTGENAPPapex_ws_name6VARCHAR230UWorkspace name (Upper Case) for the generated APEX pages
TAB_COLS                      DTGENAPPapex_app_name7VARCHAR230Application name (Mixed Case) for the generated APEX pages
TAB_COLS                      DTGENAPPdbid8VARCHAR22000Database link connect string for mid-tier connections to the centralized database server.
TAB_COLS                      DTGENAPPdb_auth9VARCHAR2200Database link authorization for mid-tier connections to the centralized database server
TAB_COLS                      DTGENAPPdescription10VARCHAR21000Description of this application
...
TAB_COLS                      DTGENFLseq22XNUMBER9Sequence number for this line in the file
...
TAB_COLS                      DTGENTABts_onln_data8VARCHAR230LTablespace name for the on-line data for this table
TAB_COLS                      DTGENTABts_onln_indx9VARCHAR230LTablespace name for the on-line indexes for this table
TAB_COLS                      DTGENTABts_hist_data10VARCHAR230LTablespace name for the history data for this table
TAB_COLS                      DTGENTABts_hist_indx11VARCHAR230LTablespace name for the history indexes for this table
...
TAB_COLS                      DTGENCOLdescription15VARCHAR21000Description for this column
...
TAB_COLS                      DTGENAPPts_onln_data12VARCHAR230LDefault tableapace name for on-line data tables
TAB_COLS                      DTGENAPPts_onln_indx13VARCHAR230LDefault tablespace name for on-line indexes
TAB_COLS                      DTGENAPPts_hist_data14VARCHAR230LDefault tablespace name for history data tables
TAB_COLS                      DTGENAPPts_hist_indx15VARCHAR230LDefault tablespace name for history indexes
TAB_COLS                      DTGENAPPts_null_override11DTGENFLAGFlag to override all tablespace names to null
TAB_COLS                      DTGENCOLupd_res_pct14NUMBER20Percentage of column space reserved for data updates
TAB_COLS                      DTGENAPPdb_schema_exp4DTGENFLAGExplicitly define the schema owner for all database object
TAB_COLS                      DTGENAPPusr_frgn_key19VARCHAR2100LForeign Key for the "_USR" data columns in history tables. Must be of the form "table", "schema.table", "table(column)",  or "schema.table(column)".
TAB_COLS                      DTGENAPPusr_datatype18VARCHAR220UDatatype for the "_USR" data columns in history tables tables. The default value is VARCHAR2(30).
TAB_COLS                      DTGENAPPcopyright20VARCHAR232000Copyright notice that is placed in the comment header in all generated scripts
