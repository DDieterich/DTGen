...
create table applications
   ...
   LOB (copyright) STORE AS BASICFILE applications_CL20
       (CACHE LOGGING)
   ;
...
create package reserved_words_tab
is

   -- Package Reserved_Words_Tab
   --    Table functions
   --    (DML and integrity checks)
   --
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   ...
   procedure ins
      (n_id  in out  NUMBER
      ,n_keyword  in out  VARCHAR2
      ,n_rtype  in out  VARCHAR2
      );
   procedure upd
      (o_id  in  NUMBER
      ,o_keyword  in  VARCHAR2
      ,n_keyword  in out  VARCHAR2
      ,o_rtype  in  VARCHAR2
      ,n_rtype  in out  VARCHAR2
      );
...
end reserved_words_tab;
/
select 'RESERVED_WORDS_TAB' as "Package:"
 from  user_errors
 where name  = 'RESERVED_WORDS_TAB'
  and  type  = 'PACKAGE'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
create sequence reserved_words_seq;
...
comment on table reserved_words is 'PL/SQL reserved words and keywords';

comment on column reserved_words.id is 'Surrogate Primary Key for these reserved_words';
comment on column reserved_words.keyword is 'PL/SQL reserved word or keyword';
comment on column reserved_words.rtype is 'Type of PL/SQL reserved word or keyword';

alter table reserved_words add constraint reserved_words_pk
   primary key (id);

/***  ACTIVE Foreign Keys  ***/

/***  ACTIVE Indexes  ***/
alter table reserved_words add constraint reserved_words_nk
    unique (keyword
           );
create index reserved_words_ix1
    on reserved_words
           (rtype
           );
...
/***  ACTIVE Audit Foreign Key Indexes  ***/
-- create index reserved_words_ia1 on reserved_words(aud_beg_usr);
-- create index reserved_words_ia2 on reserved_words(aud_beg_dtm);

create package body reserved_words_tab
is

-- Package Body Reserved_Words_Tab
--    Table functions
--    (DML and integrity checks)
--
--    Generated by DTGen (http://code.google.com/p/dtgen)
...
procedure check_rec
      (id  in  NUMBER
      ,keyword  in out  VARCHAR2
      ,rtype  in out  VARCHAR2
      )
is
begin
   -- Fold the case, as needed
   ...
   then
      ...
      then
         raise_application_error(-20003, 'reserved_words_tab.check_rec(): keyword must be lower case.');
      end if;
      if rtype != lower(rtype)
      then
         raise_application_error(-20003, 'reserved_words_tab.check_rec(): rtype must be lower case.');
      end if;
   end if;
   --  Check for NOT NULL
   if keyword is null
   then
      raise_application_error(-20004, 'reserved_words_tab.check_rec(): keyword cannot be null.');
   end if;
   if rtype is null
   then
      raise_application_error(-20004, 'reserved_words_tab.check_rec(): rtype cannot be null.');
   end if;
   -- Check for Domain Values
   if rtype not in ('reserved', 'keyword')
   then
      raise_application_error(-20005, 'reserved_words_tab.check_rec(): rtype must be one of ("reserved", "keyword").');
   end if;
   -- Custom Constraint Checks
   --  Set eff_beg_dtm, if needed
end check_rec;
...
begin
   ...
   if n_id is null
   then
      select reserved_words_seq.nextval
       into  n_id
       from  dual;
   end if;
   check_rec (n_id
...
begin
   ...
   then
      ...
      raise_application_error(-20008, 'reserved_words_tab.upd(): Must update one of'
            || ' keyword'
            || ', rtype'
            || '.');
   end if;
...
/
select 'RESERVED_WORDS_TAB' as "Package Body:"
 from  user_errors
 where name  = 'RESERVED_WORDS_TAB'
  and  type  = 'PACKAGE BODY'
  and  rownum = 1;
...
alter table reserved_words modify keyword
   constraint reserved_words_nn1 not null;
alter table reserved_words modify rtype
   constraint reserved_words_nn2 not null;
alter table reserved_words add constraint reserved_words_fld1
   check (keyword = lower(keyword));
alter table reserved_words add constraint reserved_words_dm1
   check (rtype in ('reserved', 'keyword'));

CREATE TRIGGER reserved_words_bi
   BEFORE INSERT
   ON reserved_words FOR EACH ROW
begin

   -- Trigger Reserved_Words_Bi
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:51:11 PM

   -- util.log('Trigger reserved_words_bi');
   if glob.get_db_constraints
   then
      reserved_words_tab.ins
         (:new.id
         ,:new.keyword
         ,:new.rtype
         );
   end if;
end reserved_words_bi;
/
select 'RESERVED_WORDS_BI' as "Trigger:"
 from  user_errors
 where name  = 'RESERVED_WORDS_BI'
  and  type  = 'TRIGGER'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
CREATE TRIGGER reserved_words_bu
   BEFORE UPDATE
   ON reserved_words FOR EACH ROW
begin

   -- Trigger Reserved_Words_Bu
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:51:11 PM

   -- util.log('Trigger reserved_words_bu');
   if glob.get_db_constraints
   then
      reserved_words_tab.upd
         (:old.id
         ,:old.keyword
         ,:new.keyword
         ,:old.rtype
...
end reserved_words_bu;
/
select 'RESERVED_WORDS_BU' as "Trigger:"
 from  user_errors
 where name  = 'RESERVED_WORDS_BU'
  and  type  = 'TRIGGER'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
CREATE TRIGGER reserved_words_bd
   BEFORE DELETE
   ON reserved_words FOR EACH ROW
begin

   -- Trigger Reserved_Words_Bd
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:51:11 PM

   -- util.log('Trigger reserved_words_bd');
   if glob.get_db_constraints
   then
      reserved_words_tab.del
         (:old.id
         ,:old.keyword
         ,:old.rtype
         );
   end if;
end reserved_words_bd;
/
select 'RESERVED_WORDS_BD' as "Trigger:"
 from  user_errors
 where name  = 'RESERVED_WORDS_BD'
  and  type  = 'TRIGGER'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
is
   ...
   --
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:51:43 PM

   procedure ins
      (n_id  in out  NUMBER
      ,n_file_id  in out  NUMBER
      ,n_files_nk1  in  NUMBER
...
is
   ...
   function get_id
      (file_lines_nk1  in  NUMBER
      ,file_lines_nk2  in  NUMBER
   ...
   procedure ins
      (n_id  in out  NUMBER
      ,n_file_id  in out  NUMBER
      ,n_files_nk1_in  in  NUMBER  default null
...
is
   o_file_id  NUMBER(38);
   o_files_nk1  NUMBER(38);
   o_files_nk2  VARCHAR2(30);
   n_files_nk1  NUMBER(38);
...
is
   ...
   --
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:51:46 PM

   procedure ins
      (n_id  in out  NUMBER
      ,n_domain_id  in out  NUMBER
      ,n_domains_nk1  in  NUMBER
...
is
   ...
   function get_id
      (domain_values_nk1  in  NUMBER
      ,domain_values_nk2  in  NUMBER
   ...
   procedure ins
      (n_id  in out  NUMBER
      ,n_domain_id  in out  NUMBER
      ,n_domains_nk1_in  in  NUMBER  default null
...
is
   ...
   o_domains_nk1  NUMBER(38);
   o_domains_nk2  VARCHAR2(5);
   n_domains_nk1  NUMBER(38);
...
is
   ...
   --
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:51:50 PM

   procedure ins
      (n_id  in out  NUMBER
      ,n_table_id  in out  NUMBER
      ,n_tables_nk1  in  NUMBER
      ...
      ,n_fk_prefix  in out  VARCHAR2
      ,n_fk_table_id  in out  NUMBER
      ,n_fk_tables_nk1  in  NUMBER
      ,n_fk_tables_nk2  in  VARCHAR2
      ,n_d_domain_id  in out  NUMBER
      ,n_d_domains_nk1  in  NUMBER
   ...
   procedure upd
      ...
      ,n_fk_prefix  in out  VARCHAR2
      ,o_fk_table_id  in  NUMBER
      ,o_fk_tables_nk1  in  NUMBER
      ,o_fk_tables_nk2  in  VARCHAR2
      ,n_fk_table_id  in out  NUMBER
      ,n_fk_tables_nk1  in  NUMBER
      ,n_fk_tables_nk2  in  VARCHAR2
      ,o_d_domain_id  in  NUMBER
      ,o_d_domains_nk1  in  NUMBER
...
is
   ...
   function get_id
      (tab_cols_nk1  in  NUMBER
      ,tab_cols_nk2  in  NUMBER
   ...
   procedure ins
      (n_id  in out  NUMBER
      ,n_table_id  in out  NUMBER
      ,n_tables_nk1_in  in  NUMBER  default null
      ...
      ,n_fk_prefix  in out  VARCHAR2
      ,n_fk_table_id  in out  NUMBER
      ,n_fk_tables_nk1_in  in  NUMBER  default null
      ,n_fk_tables_nk2_in  in  VARCHAR2  default null
      ,n_d_domain_id  in out  NUMBER
      ,n_d_domains_nk1_in  in  NUMBER  default null
...
is
   ...
   o_tables_nk1  NUMBER(38);
   o_tables_nk2  VARCHAR2(5);
   n_tables_nk1  NUMBER(38);
...
is
   ...
   --
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:51:54 PM

   procedure ins
      (n_id  in out  NUMBER
      ,n_tab_col_id  in out  NUMBER
      ,n_tab_cols_nk1  in  NUMBER
      ,n_tab_cols_nk2  in  NUMBER
   ...
   procedure upd
      (o_id  in  NUMBER
      ,o_tab_col_id  in  NUMBER
      ,o_tab_cols_nk1  in  NUMBER
      ,o_tab_cols_nk2  in  NUMBER
...
is
   ...
   function get_id
      (indexes_nk1  in  NUMBER
      ,indexes_nk2  in  NUMBER
      ,indexes_nk3  in  NUMBER
   ...
   procedure ins
      (n_id  in out  NUMBER
      ,n_tab_col_id  in out  NUMBER
      ,n_tab_cols_nk1_in  in  NUMBER  default null
      ,n_tab_cols_nk2_in  in  NUMBER  default null
...
is
   ...
   o_tab_cols_nk1  NUMBER(38);
   o_tab_cols_nk2  NUMBER(38);
   o_tab_cols_nk3  VARCHAR2(25);
   n_tab_cols_nk1  NUMBER(38);
   n_tab_cols_nk2  NUMBER(38);
...
is
   ...
   function get_id
      (check_cons_nk1  in  NUMBER
      ,check_cons_nk2  in  NUMBER
...
create package reserved_words_view
is

   -- Package Reserved_Words_View
   --    View functions
   --    (DML, Foreign Keys, Paths, and )
   --
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:52:04 PM
...
end reserved_words_view;
/
select 'RESERVED_WORDS_VIEW' as "Package:"
 from  user_errors
 where name  = 'RESERVED_WORDS_VIEW'
  and  type  = 'PACKAGE'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
create view reserved_words_act
      (id
      ,keyword
      ,rtype
      )
   as select
      ...
      ,RW.keyword
      ,RW.rtype
 from             reserved_words RW
 ;
select 'RESERVED_WORDS_ACT' as "View:"
 from  user_errors
 where name  = 'RESERVED_WORDS_ACT'
  and  type  = 'VIEW'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
comment on table reserved_words_act is 'PL/SQL reserved words and keywords';

comment on column reserved_words_act.id is 'Surrogate Primary Key for these reserved_words_act';
comment on column reserved_words_act.keyword is 'PL/SQL reserved word or keyword';
comment on column reserved_words_act.rtype is 'Type of PL/SQL reserved word or keyword';

alter view reserved_words_act add constraint reserved_words_act_pk
   primary key (id) disable;


create TRIGGER reserved_words_ioi
   instead of insert on reserved_words_act
   for each row
declare
   n_id  NUMBER(38);
   n_keyword  VARCHAR2(30);
   n_rtype  VARCHAR2(8);
begin

   -- Trigger Reserved_Words_Ioi
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:52:04 PM

   n_id := :new.id;
   n_keyword := :new.keyword;
   n_rtype := :new.rtype;
   reserved_words_view.ins
      (n_id
      ,n_keyword
      ,n_rtype
      );
end reserved_words_ioi;
/
select 'RESERVED_WORDS_IOI' as "Trigger:"
 from  user_errors
 where name  = 'RESERVED_WORDS_IOI'
  and  type  = 'TRIGGER'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
create TRIGGER reserved_words_iou
   instead of update on reserved_words_act
   for each row
declare
   n_keyword  VARCHAR2(30);
   n_rtype  VARCHAR2(8);
begin
   ...
   -- Trigger Reserved_Words_Iou
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:52:04 PM

   n_keyword := :new.keyword;
   n_rtype := :new.rtype;
   reserved_words_view.upd
      (:old.id
      ,:old.keyword
      ,n_keyword
      ,:old.rtype
...
end reserved_words_iou;
/
select 'RESERVED_WORDS_IOU' as "Trigger:"
 from  user_errors
 where name  = 'RESERVED_WORDS_IOU'
  and  type  = 'TRIGGER'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
create TRIGGER reserved_words_iod
   instead of delete on reserved_words_act
   for each row
begin

   -- Trigger Reserved_Words_Iod
   --    Generated by DTGen (http://code.google.com/p/dtgen)
   --    August    06, 2012  11:52:04 PM

   reserved_words_view.del
      (:old.id
      ,:old.keyword
      ,:old.rtype
      );
end reserved_words_iod;
/
select 'RESERVED_WORDS_IOD' as "Trigger:"
 from  user_errors
 where name  = 'RESERVED_WORDS_IOD'
  and  type  = 'TRIGGER'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
create package reserved_words_dml
is

   -- Package Reserved_Words_Dml
   -- (Data Manipulation Language, Foreign Key and Path Lookup)
   ...
   function get_id
      (reserved_words_nk1  in  VARCHAR2
      ) return number;
   function get_nk
      (id_in  in  number
      ) return varchar2;
   ...
   procedure clear
      (n_buff  in out  reserved_words_ACT%ROWTYPE);
   procedure clear
      (n_buff  in out  reserved_words%ROWTYPE);
...
end reserved_words_dml;
/
select 'RESERVED_WORDS_DML' as "Package:"
 from  user_errors
 where name  = 'RESERVED_WORDS_DML'
  and  type  = 'PACKAGE'
  and  rownum = 1;
select '('||line||'/'||position||') '||text as error
 from  user_errors
...
create package body reserved_words_view
is

-- Package Body Reserved_Words_View
--    View functions
--    (DML, Foreign Keys, Paths, and )
--
--    Generated by DTGen (http://code.google.com/p/dtgen)
...
is
begin
   if util.db_object_exists('RESERVED_WORDS', 'MATERIALIZED VIEW')
   then
      raise_application_error(-20010, 'Insert not allowed on materialized view reserved_words.  Inserts on reserved_words must be performed on the central database.');
   end if;
   if not glob.get_db_constraints
   then
      reserved_words_tab.ins
...
begin
   if util.db_object_exists('RESERVED_WORDS', 'MATERIALIZED VIEW')
   then
      raise_application_error(-20010, 'Update not allowed on materialized view reserved_words.  Updates on reserved_words must be performed on the central database.');
   end if;
   if not glob.get_db_constraints
   then
      reserved_words_tab.upd
   ...
   update reserved_words RW
     set  RW.keyword = n_keyword
         ,RW.rtype = n_rtype
    where RW.id = o_id;
end upd;
----------------------------------------
...
begin
   if util.db_object_exists('RESERVED_WORDS', 'MATERIALIZED VIEW')
   then
      raise_application_error(-20010, 'Delete not allowed on materialized view reserved_words.  Deletes on reserved_words must be performed on the central database.');
   end if;
   if not glob.get_db_constraints
   then
      reserved_words_tab.del
   ...
   delete from reserved_words  RW
    where RW.id = o_id;
end del;
----------------------------------------
end reserved_words_view;
/
select 'RESERVED_WORDS_VIEW' as "Package Body:"
 from  user_errors
 where name  = 'RESERVED_WORDS_VIEW'
  and  type  = 'PACKAGE BODY'
  and  rownum = 1;
...
create package body reserved_words_dml
is

 -- Package Body Reserved_Words_Dml
 -- (Data Manipulation Language, Foreign Key and Path Lookup)
...
is
   retid  number;
begin
   select reserved_words_seq.nextval into retid from dual;
   return retid;
end get_next_id;
----------------------------------------
function get_curr_id
...
is
   retid  number;
begin
   select reserved_words_seq.currval into retid from dual;
   return retid;
end get_curr_id;
----------------------------------------
function get_id
...
begin
   select id
    into  retid
    from  reserved_words  RW
    where RW.keyword = reserved_words_nk1
    ;
   return retid;
exception
   when no_data_found
...
function tab_to_col
   ...
   return col_type
is
   -- This function is duplicated in reserved_words_POP
   cursor acur is
...
begin
   ...
   rcol := COL_TYPE
             (PAIR_TYPE('keyword', abuf.keyword)
             ,PAIR_TYPE('rtype', abuf.rtype)
                );
   close acur;
   return rcol;
end tab_to_col;
...
begin
   reserved_words_dml.ins
      (n_id => n_buff.id
      ,n_keyword => n_buff.keyword
      ,n_rtype => n_buff.rtype
      );
end ins;
----------------------------------------
procedure upd
...
/
select 'RESERVED_WORDS_DML' as "Package Body:"
 from  user_errors
 where name  = 'RESERVED_WORDS_DML'
  and  type  = 'PACKAGE BODY'
  and  rownum = 1;
