...
if [ ${SYS_CONNECT_STRING-NULL}  = "NULL" -o \
     ${TEST_CONNECT_STRING-NULL} = "NULL" -o \
...
sqlplus ${SYS_CONNECT_STRING-NULL} as sysdba > ${logfile} 2>&1 <<EOF
   ...
   @../../supp/grant_role_option UTP_APP ${DB_SCHEMA}
   @../../supp/grant_role_option UTP_APP ${DB_USER}
   @../../supp/grant_role_option UTP_APP ${MT_SCHEMA}
   @../../supp/grant_role_option UTP_APP ${MT_USER}
EOF
